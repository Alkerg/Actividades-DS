...
data "external" "global_metadata" {
  program = ["python3", "${path.root}/scripts/python/generate_global_metadata.py"]
}
...
module "simulated_apps" {
  ...
  #añado el id del despliegue

  deployment_id = data.external.global_metadata.result.deploy_id
}
...
resource "null_resource" "check_all_healths" {
  ...
  provisioner "local-exec" {
    command     = "for service_dir in $(ls -d ${path.cwd}/generated_environment/services/*/); do bash ${path.cwd}/scripts/bash/check_simulated_health.sh \"$service_dir\"; done"
    interpreter = ["bash", "-c"]
  }
